...
class LETNet(nn.Module):
    def __init__(self, classes=19, block_1=3, block_2=12, block_3=12, block_4=3, block_5 = 3, block_6 = 3):
        ...
        # DAB Block B1-down
        dilation_block_b1 = [1, 1, 2, 2, 4, 4, 8, 8, 16, 16, 32, 32]
        self.downsample_b1 = DownSamplingBlock(128, 64)
        self.DAB_Block_b1 = nn.Sequential()
        for i in range(0, block_2):
            self.DAB_Block_b1.add_module("DAB_Module_b1_" + str(i),
                                        DABModule(64, d=dilation_block_b1[i]))
        self.bn_prelu_b2 = BNPReLU(64)
        # DAB Block B2-down
        dilation_block_b2 = [1, 1, 2, 2, 4, 4, 8, 8, 16, 16, 32, 32]
        self.downsample_b2 = DownSamplingBlock(64, 32)
        self.DAB_Block_b2 = nn.Sequential()
        for i in range(0, block_3):
            self.DAB_Block_b2.add_module("DAB_Module_b2_" + str(i),
                                        DABModule(32, d=dilation_block_b2[i]))
        self.bn_prelu_b3 = BNPReLU(32)
        # DAB Block B3-up
        dilation_block_b3 = [1, 1, 2, 2, 4, 4, 8, 8, 16, 16, 32, 32]
        self.upsample_b3 = UpsampleingBlock(32, 64)
        self.DAB_Block_b3 = nn.Sequential()
        for i in range(0, block_3):
            self.DAB_Block_b3.add_module("DAB_Module_b3_" + str(i),
                                        DABModule(32, d=dilation_block_b3[i]))
        self.bn_prelu_b4 = BNPReLU(64)
        # DAB Block B4-up
        dilation_block_b4 = [1,1, 2, 2, 4, 4, 8, 8, 16, 16,32,32]
        self.upsample_b4 = UpsampleingBlock(64, 128)
        self.DAB_Block_b4 = nn.Sequential()
        for i in range(0, block_2):
            self.DAB_Block_b4.add_module("DAB_Module_b4_" + str(i),
                                        DABModule(64, d=dilation_block_b4[i]))
        self.bn_prelu_b5 = BNPReLU(128)        
        ...
        # DAB Block A3B
        #dilation_block_3 = [2, 5, 7, 9, 13, 17]
        dilation_block_3B = [1,1, 2, 2, 4, 4, 8, 8, 16, 16,32,32]
        self.downsample_3B = DownSamplingBlock(64, 32)
        self.DAB_Block_3B = nn.Sequential()
        for i in range(0, block_3):
            self.DAB_Block_3B.add_module("DAB_Module_3B_" + str(i),
                                        DABModule(32, d=dilation_block_3[i]))
        self.bn_prelu_4B = BNPReLU(32)
...
#DECODER
        ...
        self.LC2B = LongConnection(64, 32, 3)
    ...
    def forward(self, input):
        ...
        # DAB Block b1
        outputb1_0 = self.downsample_b1(output2)
        outputb1 = self.DAB_Block_b1(outputb1_0)
        outputb1 = self.bn_prelu_b2(outputb1)
        # print('outputb1', outputb1.shape) #([3, 64, 25, 25])
        
        # DAB Block b2
        outputb2_0 = self.downsample_b2(outputb1)
        outputb2 = self.DAB_Block_b2(outputb2_0)
        outputb2 = self.bn_prelu_b3(outputb2)
        # print('outputb2', outputb2.shape) #([3, 32, 13, 13])
        
        # DAB Block b3
        outputb3 = self.DAB_Block_b3(outputb2)
        # print('outputb3', outputb3.shape) #([3, 32, 13, 13])
        outputb3 = self.upsample_b3(outputb3 + outputb2)
        outputb3 = self.bn_prelu_b4(outputb3)
        
        
        # DAB Block b4
        outputb4 = self.DAB_Block_b4(outputb3)
        # print('outputb4', outputb4.shape) #([3, 64, 26, 26])
        outputb1 = F.interpolate(outputb1, size=outputb4.size()[2:], mode='bilinear', align_corners=False)
        outputb4 = self.upsample_b4(outputb4 + outputb1)
        outputb4 = self.bn_prelu_b5(outputb4)
        
        
        # DAB Block 3
        output3_0 = self.downsample_3(outputb4)
        output3 = self.DAB_Block_3(output3_0)
        output3 = self.bn_prelu_4(output3)
        
        # DAB Block 3B
        output3B = self.downsample_3B(output3)
        output3B = self.DAB_Block_3B(output3B)
        output3B = self.bn_prelu_4B(output3B)

#Transformer

        b, c, h, w = output3B.shape
        output4 = self.transformer1(output3B)
...
#DECODER            
        output4 = self.DAB_Block_4(output4) # 这里修改了
        output4 = self.upsample_1(output4 + (output3B))
        output4 = self.bn_prelu_5(output4)
        
        output5 = self.DAB_Block_5(output4)
        output5 = F.interpolate(output5, size=output2.size()[2:], mode='bilinear', align_corners=False)
    ...
    def update_params(self, updated_params):
        # Update the parameters of the UNet model
