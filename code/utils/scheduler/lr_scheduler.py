...
class WarmupPolyLR(_LRScheduler):
    ...
    def get_lr(self):
        ...
        #                      self.power) for base_lr in self.base_lrs]
            ...
            results = []
            for base_lr in self.base_lrs:
                if self.T_max - self.warmup_iters != 0:
                    ratio = (self.cur_iter - self.warmup_iters) / (self.T_max - self.warmup_iters)
                    if 0 <= ratio <= 1:  # Ensure ratio is within valid range
                        result = self.eta_min + (base_lr - self.eta_min) * math.pow(1 - ratio, self.power)
                        results.append(result)
                    else:
                        # Handle cases where ratio is outside [0, 1]
                        # You can raise an exception, return a default value, or handle it based on your needs
                        print('error', self.cur_iter, self.warmup_iters, self.T_max, ratio)
                else:
